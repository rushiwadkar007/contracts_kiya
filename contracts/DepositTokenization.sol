...
// File: contracts/IDepositTokenization.sol
...
// File: contracts/DepositTokenization.sol
...
interface ITokenizationRoleManager {
    ...
    function requestUserRole(string memory _losBankID,string memory roleType,string memory _name, string memory userDetails) external;
...
pragma solidity ^0.8.19;
contract DepositTokenization is
    ERC1155,
    AccessControl,
    ERC1155Pausable,
    ERC1155Burnable,
    ERC1155Supply,
    IDepositTokenization
{
    ...
    event DepositTokenizationSetUp(ITokenizationRoleManager, IMultiSigFactory);
    ...
    constructor(
        ...
        IMultiSigFactory _iMultiSigFactory
    ) ERC1155("") {
        ...
        iTokenizationRoleManager = _iTokenizationRoleManager;
        iMultiSigFactory = _iMultiSigFactory;
        emit DepositTokenizationSetUp(
            iTokenizationRoleManager,
            _iMultiSigFactory
        );
    }
    ...
    mapping(string => uint256) public certID;
    mapping(string => TokenizationStatus) public tokenizationStatus;
    mapping(string => mapping(uint256 => string)) public tokenCert;
    mapping(string => mapping(uint256 => CertificateRequest))
        public certDetails;
    mapping(string => mapping(uint256 => string[])) public adminApprovers;
    mapping(string => mapping(uint256 => string[])) public adminRejectors;
    mapping(string => mapping(uint256 => uint256)) public depositAmount;
    mapping(string => mapping(uint256 => uint256)) public depositTenure;
    ...
    function reqTokenization(
        ...
        IMultiSigWallet _iMultSigWallet,
        string memory losDepositID
    ) public returns (bool certRequest) {
        require( iTokenizationRoleManager.isUserApproved(_losBankID, _userID) == true, "Invalid User");
        certID[_losBankID] = _iMultSigWallet.incrementNonce();
        CertificateRequest memory certificateRequest = CertificateRequest({
            bankID: _losBankID,
            certificateID: _certID,
            certBlockchainTokenID: certID[_losBankID],
            certificateOwner: msg.sender,
            userID: _userID,
            certDetails: _certDetails,
            certCreationTimestamp: 0,
            totalApprovals: 0,
            totalRejections: 0,
            isApproved: false,
            isRejected: false,
        ...
        certDetails[_losBankID][certID[_losBankID]] = certificateRequest;
        depositAmount[_losBankID][certID[_losBankID]] = _depositAmount;
        depositTenure[_losBankID][certID[_losBankID]] = _depositTenure;
        // mint(msg.sender, certID[_losBankID], _tokenAmount, "");
        certRequest = true;
        tokenizationStatus[losDepositID] = TokenizationStatus.PENDING;
        return certRequest;
    }
    ...
    function approveTokenization(
        ...
        // _iMultSigWallet.executeApproval(_iMultSigWallet, 0, signature, passkey, transactionStamp); //totalTransactionValidations
        require( iTokenizationRoleManager.isAdminApproved(_losBankID, _adminID) == true, "Invalid Admin");
        require(
            certDetails[_losBankID][trxNo].certBlockchainTokenID == trxNo,
            "Invalid Token"
        );
     certDetails[_losBankID][trxNo].totalApprovals++;
        // require(_iMultSigWallet.totalTransactionValidations(trxNo) >=2, "Invalid Request1");
        //_iMultSigWallet.totalTransactionValidations(trxNo) > 2 enter this cond. later
        if ( certDetails[_losBankID][trxNo].totalApprovals > 2) {
            certDetails[_losBankID][trxNo].certCreationTimestamp = block
                .timestamp; //updated
            certDetails[_losBankID][trxNo].isApproved = true; //updated
            certDetails[_losBankID][trxNo].allVoted = true; //updated
            mint(
                certDetails[_losBankID][trxNo].certificateOwner,
                certDetails[_losBankID][trxNo].certBlockchainTokenID,
                1,
                ""
            );
            setTokenCert(
            ...
            certRequest = true;
            adminApprovers[_losBankID][trxNo].push(adminEmail);
            return certRequest;
        } else {
            if (certDetails[_losBankID][trxNo].totalApprovals == 2) {
                certDetails[_losBankID][trxNo].certCreationTimestamp = block
            ...
            certDetails[_losBankID][trxNo].isApproved = true; //updated
                ...
                certRequest = true;
                adminApprovers[_losBankID][trxNo].push(adminEmail);
                tokenizationStatus[losDepositID] = TokenizationStatus.ACCEPTED;
                return certRequest;
            } else {
                certDetails[_losBankID][trxNo].certCreationTimestamp = block
                .timestamp; //updated
    ...
    function rejectTokenization(
        ...
        require(
            ...
            certDetails[_losBankID][trxNo].totalRejections++;
        if (
            //condition adding on the basis of assumption that total admin count will be 3
            //_iMultSigWallet.totalTransactionValidations(trxNo) > 2 && enter this condition later
            certDetails[_losBankID][trxNo].totalRejections > 2
        ) {
            certDetails[_losBankID][trxNo].isRejected =  true;
            certDetails[_losBankID][trxNo].allVoted = true;
            setTokenCert(
                _losBankID,
                certDetails[_losBankID][certID[_losBankID]]
                    .certBlockchainTokenID,
                url,
                _adminID
            );
            certRequest = true;
            adminRejectors[_losBankID][trxNo].push(adminEmail);
            return certRequest;
        } else {
            if (certDetails[_losBankID][trxNo].totalRejections == 2) {
                certDetails[_losBankID][trxNo].isRejected =  true;
                setTokenCert(
                    _losBankID,
                    certDetails[_losBankID][trxNo].certBlockchainTokenID,
                ...
                certRequest = true;
                adminRejectors[_losBankID][trxNo].push(adminEmail);
                tokenizationStatus[losDepositID] = TokenizationStatus.REJECTED;
                return certRequest;
            } else {
                setTokenCert(
                    _losBankID,
    ...
    function getTokenizationRequests(
        ...
        uint256 requestID,
        uint256 _userID
    ) public view returns (CertificateRequest memory) {
        require( iTokenizationRoleManager.isUserApproved(_losBankID, _userID) == true || iTokenizationRoleManager.isAdminApproved(_losBankID, _userID) == true, "Invalid Admin or User");
        return certDetails[_losBankID][requestID];
    }

    function setTokenCert(
        string memory _losBankID,
        uint256 _certID,
        string memory _url,
        uint256 _adminID
        ...
        require( iTokenizationRoleManager.isAdminApproved(_losBankID, _adminID) == true, "Invalid Admin");
        tokenCert[_losBankID][_certID] = _url;
        return true;
    }

    function getTokenApprovers(string memory losBankID, uint256 tokenRequestID) public view returns(string[] memory){
        return adminApprovers[losBankID][tokenRequestID];
    }

    function getTokenRejectors(string memory losBankID, uint256 tokenRequestID) public view returns(string[] memory){
        return adminRejectors[losBankID][tokenRequestID];
    }

    function getDepositAmount(string memory _losBankID) public view returns(uint256){
        return depositAmount[_losBankID][certID[_losBankID]];
    }

    function getDepositTenure(string memory _losBankID) public view returns(uint256){
        return depositTenure[_losBankID][certID[_losBankID]];
    }
}
